...
export async function getProjectById(id: string) {
  ...
  if (error) {
    console.error(`プロジェクト取得エラー (ID: ${id}):`, error)
    throw error
  }
...
export async function getProjectsByStatus(status: string) {
  ...
  if (error) {
    console.error(`ステータス別プロジェクト取得エラー (${status}):`, error)
    throw error
  }
...
export async function getProjectsByDateRange(startDate: string, endDate: string) {
  ...
  if (error) {
    console.error('期間別プロジェクト取得エラー:', error)
    throw error
  }
...
export async function updateProject(id: string, updates: Partial<Project>) {
  ...
  if (error) {
    console.error(`プロジェクト更新エラー (ID: ${id}):`, error)
    throw error
  }
...
export async function deleteProject(id: string) {
  ...
  if (error) {
    console.error(`プロジェクト削除エラー (ID: ${id}):`, error)
    throw error
  }
...
export async function getProjectPayables(projectId: string) {
  ...
  if (error) {
    console.error(`買掛取得エラー (Project ID: ${projectId}):`, error)
    throw error
  }
...
export async function updateProjectPayable(id: string, updates: Partial<{
  ...
  if (error) {
    console.error(`買掛更新エラー (ID: ${id}):`, error)
    throw error
  }
...
export async function deleteProjectPayable(id: string) {
  ...
  if (error) {
    console.error(`買掛削除エラー (ID: ${id}):`, error)
    throw error
  }
