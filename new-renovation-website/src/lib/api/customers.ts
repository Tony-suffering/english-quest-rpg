...
export async function getCustomerById(id: string) {
  ...
  if (error) {
    console.error(`取引先取得エラー (ID: ${id}):`, error)
    throw error
  }
...
export async function updateCustomer(id: string, updates: Partial<Customer>) {
  ...
  if (error) {
    console.error(`取引先更新エラー (ID: ${id}):`, error)
    throw error
  }
...
export async function deleteCustomer(id: string) {
  ...
  if (error) {
    console.error(`取引先削除エラー (ID: ${id}):`, error)
    throw error
  }
...
export async function getCustomerProjectCount(customerId: string) {
  ...
  if (error) {
    console.error(`案件数取得エラー (Customer ID: ${customerId}):`, error)
    throw error
  }
