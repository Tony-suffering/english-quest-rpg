  ...
  const fetchCurrentSheet = async () => {
    ...
    try {
      ...
      if (error && error.code !== 'PGRST116') {
        console.log('粗利表の取得エラー:', error.message)
      } else if (data) {
        console.log('粗利表データ取得:', {
          id: data.id,
      ...
      console.error('Error in fetchCurrentSheet:', err)
    }
  }
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === 'PGRST116' || error.message?.includes('not found')) {
          console.log('指定された案件が見つかりません')
        } else {
          console.log('案件情報の取得エラー:', error.message || error)
        }
      } else {
        setProjectData(data)
        // 売掛金額を初期値として設定
      ...
      console.log('Error in fetchProjectData:', err)
    }
  }
  ...
  const saveSheet = async () => {
    ...
    try {
      if (selectedSheetId) {
        ...
        console.log('Inserting new sheet data:', insertData)
        ...
        if (error) {
          ...
          console.log('Successfully inserted sheet:', insertedData)
      ...
      console.error('Error in saveSheet:', err)
