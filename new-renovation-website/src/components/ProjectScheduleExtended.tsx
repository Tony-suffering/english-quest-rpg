...
export default function ProjectScheduleExtended() {
  ...
  const loadMemosFromDB = async () => {
    try {
      ...
      console.error('Failed to load memos from DB:', error);
  ...
  const saveMemoToDB = async (memoKey: string, memoValue: string, memoColor: string) => {
    try {
      ...
      console.error('Failed to save memo to DB:', error);
  ...
  const deleteMemoFromDB = async (memoKey: string) => {
    try {
      ...
      console.error('Failed to delete memo from DB:', error);
  ...
  const fetchImportantNotice = async () => {
    try {
      ...
      if (error && error.code !== 'PGRST116') { // PGRST116 = no rows returned
        console.error('Error fetching important notice:', error);
        return;
      }
  ...
  const handleSaveNotice = async () => {
    try {
      ...
      if (error) {
        console.error('Error saving important notice:', error);
  ...
  const handleDeleteNotice = async () => {
    try {
      ...
      if (error) {
        console.error('Error deleting important notice:', error);
  ...
  const handleChangeMemoColor = async (memoKey: string, colorClass: string) => {
    ...
    try {
      ...
      if (existingMemo) {
        ...
        if (error) {
          console.error('Error saving memo color to DB:', error);
        }
      }
    } catch (error) {
      console.error('Error saving memo color:', error);
    }
  };
  ...
  useEffect(() => {
    ...
    if (savedColors) {
      try {
        ...
        console.error('Error loading memo colors:', error);
      }
    }
  }, []);
  ...
  const fetchProjects = async () => {
    try {
      ...
      if (projectsData) {
        console.log('取得したプロジェクト:', projectsData);
  ...
  const getProjectsForDateGrid = (dateStr: string) => {
    return [...localProjects, ...projects].filter(p => {
      ...
      try {
        ...
        console.error('Date parsing error:', error, p);
        return false;
      }
    });
  };
