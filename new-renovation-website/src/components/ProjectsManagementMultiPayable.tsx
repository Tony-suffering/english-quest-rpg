...
const ProjectsManagementMultiPayable: React.FC = () => {
  ...
  const syncProjectToFunds = async (projectId: string) => {
    try {
      ...
      console.error('Error syncing project to funds:', error)
      // エラーは出すが、案件保存自体は成功扱い
    }
  }
  ...
  useEffect(() => {
    console.log('ProjectsManagementMultiPayable initialized');
  ...
  const fetchProjectGrossProfitSheet = async (projectId: string) => {
    try {
      ...
      if (error && error.code !== 'PGRST116') {
        console.error('Error fetching gross profit sheet:', error);
      }
  ...
  const fetchCustomers = async () => {
    ...
    try {
      ...
      console.log(`Fetched ${data?.length || 0} customers for select`);
      setCustomers(data || []);
    } catch (error: any) {
    ...
    try {
      ...
      const projectsWithPayables = await Promise.all(
        (projectData || []).map(async (project) => {
          ...
          if (payablesError) {
            console.error(`Error fetching payables for project ${project.id}:`, payablesError);
            return {
              ...project,
              project_payables: [],
      ...
      console.log(`Fetched ${projectsWithPayables.length} projects with payables`);
  ...
  const handleSaveProject = async (formData: any) => {
    try {
      console.log('=== Saving project with payables ===');
      ...
      if (formData.payables && formData.payables.length > 0) {
        ...
        if (validPayables.length > 0) {
          ...
          console.log('Saving payables:', payablesData);
          
          const { data: payableResult, error: payablesError } = await supabase
            .from('project_payables')
            .insert(payablesData)
            .select();
          
          if (payablesError) {
            console.error('Error saving payables:', payablesError);
            ...
            console.log('Payables saved successfully:', payableResult);
    ...
    try {
      ...
      if (storedData) {
        try {
          ...
          console.error('Error updating funds calendar:', err);
...
const ProjectForm: React.FC<ProjectFormProps> = ({ project, customers, onSave, onCancel }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // 有効な買掛明細のみフィルタリング  
    const validPayables = formData.payables.filter(p => 
      p.payable_customer_id && p.payable_amount && parseFloat(p.payable_amount) > 0
    );
    console.log('Form submission - payables:', formData.payables);
    console.log('Form submission - valid payables:', validPayables);
    onSave({
      ...formData,
