  ...
  const fetchProjectsWithProfit = async () => {
    try {
      ...
      if (sheetsError) {
        ...
        if (sheetsError.code === '42P01' || sheetsError.message?.includes('relation')) {
          console.log('粗利表テーブルが未作成です')
          setProjects([])
          return
        }
        console.error('Error fetching profit data:', sheetsError.message || sheetsError)
        setProjects([])
        return
      }

      if (sheetsData && sheetsData.length > 0) {
        console.log('Fetched sheets data:', sheetsData)
