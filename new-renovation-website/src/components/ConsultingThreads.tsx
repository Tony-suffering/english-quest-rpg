...
export default function ConsultingThreads() {
  ...
  const fetchThreads = async () => {
    try {
      ...
      if (threadError) {
        console.error('Thread fetch error details:', JSON.stringify(threadError, null, 2));
        console.error('Error object:', threadError);
        throw threadError;
      }
  ...
  const fetchMessages = async (threadId: string) => {
    ...
    try {
      ...
      if (error) {
        console.error('Message fetch error details:', error);
        throw error;
      }
      setMessages(data || []);
    } catch (error: any) {
  ...
  const createThread = async () => {
    ...
    try {
      ...
      if (error) {
        console.error('Thread creation error details:', JSON.stringify(error, null, 2));
      ...
      console.error('Error creating thread - Full object:', error);
      console.error('Error creating thread - Stringified:', JSON.stringify(error, null, 2));
      console.error('Error message:', error?.message);
      console.error('Error details:', error?.details);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (error) {
        console.error('Message send error details:', JSON.stringify(error, null, 2));
        console.error('Error object:', error);
        throw error;
      }
      ...
      try {
        ...
        console.error('AI回答生成エラー:', aiError);
      ...
      console.error('Error sending message - Full object:', error);
      console.error('Error sending message - Stringified:', JSON.stringify(error, null, 2));
      console.error('Error message:', error?.message);
      console.error('Error details:', error?.details);
      console.error('Error code:', error?.code);
