...
export default function QuotationsManager({ projectId, editId }: Props) {
  ...
  useEffect(() => {
    const loadProjectData = async () => {
      if (projectId && projects.length > 0 && !editId) {
        console.log('Auto-selecting project:', projectId)
        ...
        if (project) {
          ...
          if (project.receivable_customer_id) {
            ...
            console.log('Project auto-select - Customer data:', customerData)
            console.log('Project auto-select - Error:', error)
  ...
  const fetchQuotations = async () => {
    ...
    console.log('Fetched quotations:', data?.length || 0, 'items')
  ...
  const fetchProjects = async () => {
    try {
      ...
      console.log('Fetched projects with customers:', projectsWithCustomers.length)
  ...
  const handleProjectSelect = async (projectId: string) => {
    ...
    const project = projects.find(p => p.id === projectId)
    console.log('Found project for selection:', project)
    ...
    if (project) {
      ...
      if (project.receivable_customer_id) {
        ...
        console.log('Project select - Customer data:', customerData)
        console.log('Project select - Error:', error)
        
        if (customerData) {
          console.log('Customer data postal_code:', customerData.postal_code)
          console.log('Customer data address:', customerData.address)
          setFormData(prev => {
            const newData = {
              ...prev,
              customer_id: project.receivable_customer_id,
              ...
              <div style={{ width: '62%' }}>
                ...
                <div style={{ 
                  ...
                  <div style={{ fontSize: '11px', marginBottom: '4px' }}>
                    {(() => {
                      console.log('Rendering postal code, formData.customer_postal:', formData.customer_postal)
                      if (!formData.customer_postal || formData.customer_postal === '') {
                        return '〒'
                      }
                      const cleanPostal = formData.customer_postal.replace(/[^0-9]/g, '')
                      console.log('Clean postal:', cleanPostal)
