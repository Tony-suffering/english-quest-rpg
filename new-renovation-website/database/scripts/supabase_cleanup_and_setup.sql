...
-- ポリシーを削除（テーブルが存在する場合のみ）
DO $$
BEGIN
    DROP POLICY IF EXISTS "Public profiles are viewable by everyone" ON public.craftsmen_profiles;
    DROP POLICY IF EXISTS "Users can insert their own profile" ON public.craftsmen_profiles;
    DROP POLICY IF EXISTS "Users can update their own profile" ON public.craftsmen_profiles;
    DROP POLICY IF EXISTS "Public work records are viewable by everyone" ON public.work_records;
    DROP POLICY IF EXISTS "Users can insert their own work records" ON public.work_records;
    DROP POLICY IF EXISTS "Users can update their own work records" ON public.work_records;
    DROP POLICY IF EXISTS "Users can delete their own work records" ON public.work_records;
...
-- トリガーを削除（テーブルが存在する場合のみ）
DO $$
BEGIN
    DROP TRIGGER IF EXISTS update_craftsmen_profiles_updated_at ON public.craftsmen_profiles;
    DROP TRIGGER IF EXISTS update_work_records_updated_at ON public.work_records;
