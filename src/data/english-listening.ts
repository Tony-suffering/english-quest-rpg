...
export interface ExpressionPick {
    japanese: string;
    english: string;
    context?: string;
    why?: string;
...
export const listeningContents: ListeningContent[] = [
    ...
    {
        id: "rensox-cubs-2026-last-dance",
        youtubeId: "",
        title: "カブス2026 ラストダンス -- RenSox",
        description: "RenSoxによるシカゴ・カブス2026シーズンプレビュー。今のロスターで戦うラストチャンス。",
        date: "2026-02-23",
        segments: [],
        expressions: [
            {
                japanese: "惜しくも",
                english: "came up just short",
                context: "惜しくも地区第2位ということでね",
                why: "惜しいは後悔+惜しさ+敬意が一語に詰まってる。came up just shortはその全部を運べる数少ない英語"
            },
            {
                japanese: "悔しい思いをした",
                english: "left a bitter taste",
                context: "悔しい思いをしたシカゴカブス",
                why: "悔しいは多分最も翻訳不可能な日本語感情。全力でやって負けた時の焼けるような感覚。英語には一語でこれがない"
            },
            {
                japanese: "勝負の年",
                english: "make-or-break year",
                context: "今年が勝負の年である",
                why: "勝負は勝ち負けだけどサムライの覚悟が入ってる。make-or-breakも同じ崖っぷち感がある"
            },
            {
                japanese: "軒並みFAになる",
                english: "hitting free agency across the board",
                context: "主力の選手は軒並みFAになる",
                why: "軒並み=屋根が並ぶ→次から次へと全員。この画像的な表現は日本語の得意技"
            },
            {
                japanese: "想定内",
                english: "saw that coming",
                context: "正直想定内だったのかな",
                why: "ホリエモンが有名にした語。内という空間表現が日本語的。saw that comingは同じ余裕を含んだ英語"
            },
            {
                japanese: "あれよあれよと",
                english: "before you knew it",
                context: "あれよあれよと打ちまくって31ホーマー",
                why: "繰り返しのオノマトペで速さ+見てるしかない無力感を表現。日本語にしかできない芸当"
            },
            {
                japanese: "完全に脱皮した",
                english: "completely leveled up",
                context: "5ツールプレイヤーへと完全に脱皮した",
                why: "脱皮=蛇が皮を脱ぐ。improvedじゃなくて別人になった感じ。日本語スポーツ実況の動物比喩は生々しい"
            },
            {
                japanese: "不動の守護神",
                english: "the undisputed closer",
                context: "不動の守護神になりつつある",
                why: "クローザーを守護神と呼ぶ。英語はcloserだけ。重さが全然違う。神って言っちゃう日本語の大げさパワー"
            },
            {
                japanese: "玄人好み",
                english: "one for the connoisseurs",
                context: "クロート好みのリリーフという評価",
                why: "素人にはわからないがプロが唸る。この概念を一語で言える日本語。英語だとconnoisseurを引っ張り出すしかない"
            },
            {
                japanese: "いい誤算",
                english: "the best kind of surprise",
                context: "いい誤算になったな",
                why: "誤算=計算ミス。でもいい誤算=予想を超えた嬉しい裏切り。この2語の組み合わせの美しさ"
            }
        ]
    },
    // ===== レッズ2026 選定プロセス =====
    //
    // 全17分のスクリプトから表現を拾った。内容(選手の成績)ではなく言い回しだけを見た。
    //
    // ■ 候補として拾った表現 (30個以上):
    //   ドキドキさせてくれる / バウンスバック候補 / アン価で獲得 / 探し回った挙げく /
    //   土壇場で / 方針を転換して / お客さんが呼べる選手 / 既存戦力のまま戦う /
    //   東打の柱がバシッと立ってる / とんがった部分も見当たらない / 一定のパンチ力 /
    //   便利な選手 / 壁としての能力を発揮 / ポーカーフェイス職人肌 / 鉄砲玉みたいな送球 /
    //   ビビらないか心配 / バガ型(鉄砲型) / 均してみると / 守備のプレッシャーから解放されて /
    //   バットスピードでボールを潰す / ただのブブ丸ではない選球眼 / 払拭する復活と完走 /
    //   鉄腕としての価値を証明 / 反動が来る / 非本塁打のゆり戻し / 打車を翻弄し続けた /
    //   チーム全員を差し出さない限りノーだ / ドラフト外で入団 / 若手との融合 /
    //   不動の〜 / 一気に落ちてしまった
    //
    // ■ 除外した理由:
    //   - 野球用語そのもの (バウンスバック、パンチ力、ドラフト外) → 言語ギャップが薄い
    //   - カタカナ英語の逆輸入 (バウンスバック=bounce back) → 翻訳する意味がない
    //   - 内容依存 (チーム全員を差し出さない限りノーだ) → 面白いけど汎用性がない
    //   - 類似候補 (鉄腕/鉄砲玉/バガ型 → 全部「鉄」系、ビビるだけ残した)
    //   - 壁/翻弄/不動 → 日本語として普通すぎる。ギャップが弱い
    //
    // ■ 選んだ10個の基準:
    //   1. 日本語と英語で「感覚の入口」が違うもの (心臓vs椅子、尖るvs飛び出す)
    //   2. 日常会話でも使える汎用性 (土壇場、払拭、均す → 野球以外でも使う)
    //   3. 身体感覚の差 (肌=性格、ビビる=電気、反動=バネ)
    //   4. 品格・文化の差 (呼べるvsbutts in seats)
    //   5. 10個の中でジャンルが被らないこと
    //
    // ■ 迷って落としたもの:
    //   - 「チーム全員を差し出さない限りノーだ」→ 名言だけど一発ネタ。日常で使えない
    //   - 「鉄腕」→ iron armは直訳できるのでギャップが弱い。workhorseの方が面白いが別の話になる
    //   - 「守備のプレッシャーから解放されて」→ freed fromで直訳できる。ギャップ不足
    //   - 「一気に落ちてしまった」→ fell off a cliffで対応できるが「一気」の呼吸感が消える。惜しい
    // =====
    {
        id: "rensox-reds-2026-big-red-machine",
        youtubeId: "",
        title: "レッズ2026 ビッグレッドマシン -- RenSox",
        description: "RenSoxによるシンシナティ・レッズ2026シーズンプレビュー。スモールマーケットの補強戦略と若手融合。",
        date: "2026-02-24",
        segments: [],
        expressions: [
            {
                japanese: "ドキドキさせてくれる",
                english: "Man, this team keeps you on the edge of your seat every single time",
                context: "毎回毎回ドキドキさせてくれるチームではあるんですけども",
                why: "英語には心臓の音で感情を表す単語がない。ゼロ。だから椅子の端に座るっていう姿勢で誤魔化す。日本語は心臓に正直。英語は家具に頼る"
            },
            {
                japanese: "土壇場で",
                english: "They pulled the trigger at the eleventh hour and landed Suarez",
                context: "土壇場で取ってきたのがスアレス",
                why: "土壇場の元ネタは処刑台。首が飛ぶ直前。英語のeleventh hourは時計の11時。命がかかってるvs遅刻しそう。緊迫感のスケールが3段階違う"
            },
            {
                japanese: "探し回った挙げく",
                english: "After scouring high and low, they finally grabbed him at the last second",
                context: "探し回った挙げくですね、土壇場で取ってきた",
                why: "挙げくは最高の接続詞。疲れた、諦めかけた、でも続けた。3つが2文字に入ってる。英語のafter all thatが一番近いけど汗のにおいがしない"
            },
            {
                japanese: "とんがった部分も見当たらない",
                english: "Solid team, but nothin' that really pops, you know?",
                context: "とんがった部分も特にあんまり見当たらない",
                why: "日本語は個性を形で表す。尖ってる=目立つ。丸い=穏やか。英語はpopで音にするしかない。物の形で人を描ける日本語がたまにすごく正確"
            },
            {
                japanese: "職人肌",
                english: "He's an old-school craftsman type -- stone-faced, all business",
                context: "ポーカーフェイス職人肌の選手",
                why: "肌って言うのがヤバい。性格が皮膚にあるって言ってる。英語にはtemperament-as-textureの概念がない。触ってわかる人間性。日本語しかできない芸当"
            },
            {
                japanese: "払拭する",
                english: "This was the season that put last year's mess completely behind him",
                context: "前年を払拭する復活と完走のシーズン",
                why: "払って、拭く。2動作。手を動かして汚れを物理的に取る。英語はput it behind himで背中の後ろに置くだけ。掃除してない。だから英語の方が再発する"
            },
            {
                japanese: "反動が来る",
                english: "Eighty games last year? The regression's definitely comin' for that arm",
                context: "反動が26年に来るんじゃないか",
                why: "バネを引っ張りすぎたら戻る。当たり前の物理。英語のregressionは統計レポートの言葉で体温がない。日本語で反動って言うとバネが見える。英語で言うとExcelが見える"
            },
            {
                japanese: "均してみると",
                english: "When you average it all out, he was... fine. Just fine.",
                context: "均してみるとまあまあまあまあ",
                why: "均す=でこぼこの地面をローラーで平らにする。日本語は統計を土木工事にする天才がいる。英語のaverageは数学の授業。均すは工事現場。汗の量が違う"
            },
            {
                japanese: "お客さんが呼べる選手",
                english: "Suarez puts butts in seats -- that's half the reason they signed him",
                context: "お客さんが呼べると、古巣に戻ってくる",
                why: "呼べる。声をかけたら来てくれる。丁寧。英語はbutts in seats。尻を椅子に叩きつける。同じ概念なのに品格の差が国際問題レベル"
            },
            {
                japanese: "ビビらないか心配",
                english: "A hundred-mile throw from short to first? Hope the rookie doesn't flinch",
                context: "新人にそんな鉄砲玉みたいな送球をしてビビらないか心配",
                why: "ビビるは体がビリビリ震える擬音からの俗語。flinchは反射で体が引く動き。日本語の恐怖は電気。英語の恐怖は逃走。震えない英語圏、逃げない日本語圏"
            }
        ]
    },
    // ===== ホワイトソックス2026 選定プロセス =====
    //
    // 18分のRenSoxスクリプトから表現を拾った。
    // 再建球団なので「待つ」「育てる」「我慢」系の表現が多い。
    // 内容(選手成績)ではなく言い回しだけを見た。
    //
    // ■ 候補として拾った表現 (30個以上):
    //   再建ど真ん中 / 希望の兆しが見えてきた / もう少し待ってくれ / 本格再建を始めるべく /
    //   含みながらの契約 / やっとと言いますか放出して / 結果よりも過程が大事 /
    //   保守的な考え方で運営 / 歴史的な超豊作 / 匂いがプンプンしている / 返してくんねえか /
    //   完全に無理って言われた / 温水洗浄便座をつけてくれ / お尻が痒くなっちゃう /
    //   卓越した選球眼 / 圧倒的なパワーを披露 / 最悪契約 / ピリオドを打つ...丸みたいな /
    //   信辣な文章 / 浮いた資金の投資先 / 運に助けられた / 評価を覆返す /
    //   潜在能力の高さ / エース級への飛躍 / 変革がどうしても必要 / 並み高評価のオフ /
    //   噂が絶えなかった / 場面場面では活躍 / 白い靴下になってしまった
    //
    // ■ 除外した理由:
    //   - カタカナ英語の逆輸入 (プロスペクト、ハイレバレッジ、FIP、コンテンド) → 翻訳する意味なし
    //   - 文化ネタ (温水洗浄便座、お尻が痒い) → 面白いけど言語ギャップではなく文化ギャップ
    //   - 短すぎ/単語レベル (完全に無理、卓越した、潜在能力) → 一語で語るには薄い
    //   - 意味が直訳できる (飛躍=leap、潜在能力=potential、変革=transformation) → ギャップ弱い
    //   - 内容依存 (白い靴下になってしまった) → ジョークとして面白いが汎用性なし
    //   - 前回と被る (噂が絶えなかった → レッズの「探し回った挙げく」と探索系で近い)
    //
    // ■ 選んだ10個の基準:
    //   1. 日本語の感覚的入口が英語にない (匂い→直感、兆し→占い、ど→強調)
    //   2. 身体・五感メタファーの差 (含む=口、匂い=鼻、ど真ん中=的)
    //   3. 再建球団の語彙として汎用性がある (仕事や人生の再建でも使える)
    //   4. 10個の中でジャンルが被らないこと
    //   5. whyフィールドで「へえ」と言わせられるもの
    //
    // ■ 迷って落としたもの:
    //   - 「温水洗浄便座をつけてくれ」→ 村上エピソードとして最高だが言語分析にならない
    //   - 「変革がどうしても必要」→ transformation is neededで直訳できる。ど真ん中と「変化」系で被る
    //   - 「信辣な文章」→ scathing write-upで対応できるが、信辣の「辛い」が効いてない英語が惜しい。次回に温存
    //   - 「浮いた資金の投資先」→ freed-up cashで直訳寄り。ただ「浮いた」の軽さが英語にない。惜しい
    // =====
    {
        id: "rensox-whitesox-2026-rebuild",
        youtubeId: "",
        title: "ホワイトソックス2026 白下 -- RenSox",
        description: "RenSoxによるシカゴ・ホワイトソックス2026シーズンプレビュー。121敗からの再建、村上加入、ドラフト全体1位指名権。",
        date: "2026-02-25",
        segments: [],
        expressions: [
            {
                japanese: "再建ど真ん中",
                english: "right smack in the middle of a full-blown rebuild",
                context: "相変わらず再建ど真ん中のこの球団",
                why: "ど真ん中はダーツの的のど真ん中。再建を的に例えてる。英語はsmack in the middleで殴る音。日本語は照準、英語は暴力。目指してるvs殴ってる"
            },
            {
                japanese: "希望の兆しが見えてきた",
                english: "you could finally start to see some light at the end of the tunnel",
                context: "だんだん希望の兆しが見えてきたかなと",
                why: "兆し=占いの言葉。未来が予兆として届く。英語はlight at the end of the tunnel。日本語の希望は神社から届く。英語の希望はトンネルの出口で待ってる。届くvs歩く"
            },
            {
                japanese: "匂いがプンプンしている",
                english: "this kid just SCREAMS future franchise cornerstone",
                context: "今後の象徴になりそうな匂いがプンプンしている",
                why: "プンプン=臭い時のオノマトペ。才能を鼻で嗅ぐ。英語はscreamで才能が叫ぶ。日本語の直感は嗅覚、英語の直感は聴覚。鼻が利くvs耳が良い"
            },
            {
                japanese: "含みながらの契約",
                english: "a deal loaded with subtext",
                context: "含みながらの契約の中身になった感じがしますね",
                why: "含む=口の中に入れたまま飲み込まない状態。言ってないけど口の中にある。英語のsubtextはテキストの下。日本語は口腔内、英語は地下。隠し場所が全然違う"
            },
            {
                japanese: "返してくんねえか",
                english: "come on man, just give 'im back already",
                context: "ボストンがこのオフに返してくんねえかということでね",
                why: "くんねえか=くれないか→くんねえか。3段階崩れた敬語。GMがこんな言い方するわけないけど、連ソがこう訳すことで懇願のみっともなさが出る。崩し方に愛がある"
            },
            {
                japanese: "丸みたいな...そういう役割程度しか果たせてない",
                english: "basically just a period at the end of a sentence -- that's it, that's all he is",
                context: "チーム全体のピリオドを打つような...丸みたいなそういう役割程度しか果たせてない",
                why: "選手を句読点に例える残酷さ。しかも原文自体がperiodと丸の二言語で攻めてる。日本語解説者が英語比喩を使って日本語に着地させる高等技術"
            },
            {
                japanese: "歴史的な超豊作",
                english: "a historically stacked draft class",
                context: "26年のドラフトは歴史的な超豊作と言われていて",
                why: "豊作=米が大量に穫れた年。才能を稲に見立てる農耕民族の発想。英語のstackedは積み上がった荷物。日本人は田んぼで選手を育てる。アメリカ人は倉庫に選手を積む"
            },
            {
                japanese: "運に助けられた部分もある",
                english: "some of that was fool's gold -- he got bailed out by luck",
                context: "実は運に助けられたっていうような指摘もある",
                why: "助けられた=恩を受けた。運が人格を持って助けに来る。英語のbailed outは保釈金を払ってもらう。日本語の運は恩人、英語の運は弁護士"
            },
            {
                japanese: "結果よりも過程が大事",
                english: "it's about the process, not the results",
                context: "GMが兼ねてから言ってるように結果よりも過程が大事なんだと",
                why: "過程=通り過ぎる道のり。GMがこれを言うとき日米で受け取り方が全然違う。日本なら美学として響く。アメリカならもう少し数字見せろと言われる。同じ言葉で信頼度が逆転する"
            },
            {
                japanese: "保守的な考え方で運営している",
                english: "the front office is still runnin' things super old-school",
                context: "オーナーとかフロントが基本的にはすごく保守的な考え方で運営をしている",
                why: "保守=守りを保つ。城を守る武士の語彙。英語のold-schoolは学校が古い。日本語の保守は誇りを含む。英語のold-schoolは苦笑を含む。同じ意味なのに体温が逆"
            }
        ]
    },
    {
        id: "mlb-trades-2025-offseason",
        youtubeId: "yVn54g6uP6A",
        title: "25 Offseason MLB Trades - RenSox Breakdown",
        description: "RenSox@MLB情報局による2025年オフシーズンに成立した厳選23トレードの解説。各トレードの背景、評価、チームへの影響を詳しく分析。",
        date: "2026-02-28",
        segments: [
            {
                startTime: 4168,
                english: "There are obviously cases where it doesn't work out, but this is my evaluation as of right now,",
                japanese: "ねえかってこともね、当然あるとは思いますけども、ま、現時点の評価とで、"
            },
            {
                startTime: 4175,
                english: "and keep in mind this is purely RenSox's personal opinion. I'm just an amateur after all,",
                japanese: "しかもあくまでレンソックスの個人的な見解であるということをですね、ま、是非加味していただいて、私も結局素人ですからね、"
            },
            {
                startTime: 4181,
                english: "so please don't come at me in the comments saying 'this is wrong, that's wrong.' Just putting that disclaimer out there,",
                japanese: "皆さんあの、あんまりコメントでこれへ、これへっていうのやめてくださいね。ということも一応ですね、念置きをして"
            },
            {
                startTime: 4187,
                english: "hedging my bets here, but to everyone watching this final screen, I'd like to say a few words.",
                japanese: "保険をかけながらですね、ま、この最後のね、この画像を見ている皆さんには、え、言葉をかけたいなという風に思います。"
            },
            {
                startTime: 4193,
                english: "So with that, see you in the next video. Bye. Goodbye. This was RenSox.",
                japanese: "ということでね、また次回の動画でお会いしましょう。バイ。さよなら。レンソックスでした。"
            }
        ],
        expressions: [
            {
                japanese: "現時点の評価",
                english: "my evaluation as of right now",
                context: "トレードの結果を数ヶ月後に振り返ると評価が変わる前提で話している",
                why: "as of right now = 今この瞬間の。日本語の「現時点」は公式感がある。英語のas of right nowはもっとカジュアルで「今んとこはね」くらいの温度"
            },
            {
                japanese: "あくまで個人的な見解",
                english: "purely my personal opinion",
                context: "批判コメントへの予防線として使っている",
                why: "あくまで = purely/strictly。「あくまで」は日本語の免責の定番。英語ではjust my takeとかIMOでもっと軽く済ませることが多い"
            },
            {
                japanese: "念置きをして保険をかける",
                english: "putting that disclaimer out there, hedging my bets",
                context: "視聴者に対して「怒らないでね」と予防線を張る場面",
                why: "hedge my bets = 保険をかける。ギャンブル由来の表現。日本語の「保険」は生命保険的な安心感。英語のhedgeは生垣=両側に賭ける=リスク分散"
            },
            {
                japanese: "これへ、これへっていうのやめてください",
                english: "don't come at me in the comments saying 'this is wrong'",
                context: "コメント欄で「この評価は間違い」と指摘してくる人への牽制",
                why: "come at me = 突っかかってくる。日本語は「やめてください」で丁寧に制止。英語のdon't come at meはもっと対決姿勢を感じる"
            },
            {
                japanese: "また次回の動画でお会いしましょう",
                english: "see you in the next video",
                context: "YouTube動画の定番クロージング",
                why: "「お会いしましょう」は敬語で丁寧。英語のsee youは友達感覚。日本のYouTuberは丁寧語で締める文化、英語圏はcatch you laterくらい軽い"
            }
        ]
    }
];
