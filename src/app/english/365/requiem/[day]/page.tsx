...
import { EP01_WORDS_REVIEW, Ep01WordReview } from '@/data/english/365/episodes/ep01-words-review';
...
function toWordEntry(w: Ep01WordReview, masteryOverride?: number): WordEntry {
    return {
        id: w.id,
        phrase: w.phrase,
        type: w.type,
        meaning: w.meaning,
        mastery_level: masteryOverride ?? w.mastery_level,
        review_sentence: w.review_sentence,
        review_sentence_ja: w.review_sentence_ja,
        review_idiom: w.review_idiom,
        review_idiom_meaning: w.review_idiom_meaning,
    };
}

export default function Requiem365DayPage() {
    const params = useParams();
    const router = useRouter();
    const searchParams = useSearchParams();
    const dayParam = parseInt(params?.day as string, 10) || 1;
    ...
    useEffect(() => {
        const savedSpeed = localStorage.getItem('requiem365_speed');
        const savedRepeatMode = localStorage.getItem('requiem365_repeatMode');
        const savedShuffle = localStorage.getItem('requiem365_shuffle');
        const savedPlayMode = localStorage.getItem('requiem365_dayPlayMode');
    ...
    useEffect(() => {
        ...
        localStorage.setItem('requiem365_speed', speed.toString());
    }, [speed, settingsLoaded]);

    useEffect(() => {
        if (!settingsLoaded) return;
        localStorage.setItem('requiem365_repeatMode', repeatMode.toString());
    }, [repeatMode, settingsLoaded]);

    useEffect(() => {
        if (!settingsLoaded) return;
        localStorage.setItem('requiem365_shuffle', isShuffle.toString());
    }, [isShuffle, settingsLoaded]);

    useEffect(() => {
        if (!settingsLoaded) return;
        const playMode = !autoNextDay ? 'manual' : shuffleDays ? 'shuffle' : 'auto';
        localStorage.setItem('requiem365_dayPlayMode', playMode);
    }, [autoNextDay, shuffleDays, settingsLoaded]);

    // Load words for current day from local data + localStorage mastery
    useEffect(() => {
        const dayWords = EP01_WORDS_REVIEW.filter(w => w.day === dayParam);
        const entries = dayWords.map(w => {
            const savedMastery = localStorage.getItem(`requiem365_mastery_${w.id}`);
            const mastery = savedMastery !== null ? parseInt(savedMastery) : 0;
            return toWordEntry(w, mastery);
        });
        setWords(entries);
        setCurrentIndex(0);
    ...
    const getPlayableWords = (): WordEntry[] => {
        return words;
    };
    ...
    const getNextIndex = (current: number): number => {
        ...
        if (next >= playable.length) {
            ...
            if (autoNextDayRef.current && TOTAL_DAYS > 1) {
    ...
    const goToNextDay = () => {
        ...
        if (shuffleDaysRef.current) {
            nextDay = Math.floor(Math.random() * TOTAL_DAYS) + 1;
        } else {
            nextDay = dayParam + 1;
            if (nextDay > TOTAL_DAYS) nextDay = 1;
        }

        router.push(`/english/365/requiem/${nextDay}?autoplay=true`);
    };
    ...
    const goToPrevDay = () => {
        ...
        let prevDay = dayParam - 1;
        if (prevDay < 1) prevDay = TOTAL_DAYS;

        router.push(`/english/365/requiem/${prevDay}`);
    };
    ...
    const cycleMastery = (wordId: string, e: React.MouseEvent) => {
        e.stopPropagation();
        ...
        localStorage.setItem(`requiem365_mastery_${wordId}`, next.toString());
    };

    const openVocabModal = (word: WordEntry, e: React.MouseEvent) => {
        e.stopPropagation();
    ...
    const openRegisterPopup = (word: WordEntry, type: 'phrase' | 'idiom', e: React.MouseEvent) => {
        e.stopPropagation();
        const today = new Date();
            ...
            <div style={{ padding: '24px', maxWidth: '600px', margin: '0 auto' }}>
                ...
                {playable.length > 0 && (
                    <div style={{
                        ...
                        {/* Now Playing actions: mastery + vocab */}
                        {currentWord && (
