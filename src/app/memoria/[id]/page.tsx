...
export default function MemoriaDetailPage() {
    ...
    useEffect(() => {
        ...
        if (memId) {
            setCompleted(localStorage.getItem(`memoria_${memId}_completed`) === 'true');
        }
    }, [params.id]);
    ...
    const toggleCompleted = () => {
        ...
        setCompleted(next);
        if (next) {
            localStorage.setItem(`memoria_${memId}_completed`, 'true');
        } else {
            localStorage.removeItem(`memoria_${memId}_completed`);
        }
    };
        ...
        const loadAllEntries = async () => {
            ...
            const journalMemoriaEntries: MemoriaEntry[] = (journalEntries as any[])
                ...
                .map(e => ({
                    ...
                    createdAt: e.conversationData!.generatedAt,
                    updatedAt: e.conversationData!.generatedAt,
            ...
            const tangentMemoriaEntries: MemoriaEntry[] = (journalEntries as any[])
                .filter(e => e.tangentData)
                .map(e => ({
                    id: `tangent-${e.id}`,
                    date: e.date,
                    title: e.title,
                    content: e.summary,
                    conversation: e.tangentData,
                    tone: e.tangentData!.tone,
                    createdAt: e.tangentData!.generatedAt,
                    updatedAt: e.tangentData!.generatedAt,
                    tags: [...e.businessTags, ...e.techTags],
                }));
    ...
    useEffect(() => {
        ...
        const loadEntry = async () => {
            ...
            if (id.startsWith('party-')) {
            ...
            } else if (id.startsWith('gamenight-')) {
                const gameNightEntry = gameNightEntries.find(e => e.id === id);
                if (gameNightEntry) {
                    loadedEntry = {
                        id,
                        date: gameNightEntry.date,
                        title: gameNightEntry.title,
                        content: gameNightEntry.content,
                        conversation: gameNightEntry.conversation,
                        tone: gameNightEntry.tone,
                        series: gameNightEntry.series,
                        seriesTitle: gameNightEntry.seriesTitle,
                        createdAt: gameNightEntry.createdAt,
                        updatedAt: gameNightEntry.updatedAt,
                        tags: gameNightEntry.tags,
                    };
                }
            } else if (id.startsWith('antiques-')) {
                const antiquesEntry = antiquesHouseEntries.find(e => e.id === id);
                if (antiquesEntry) {
                    loadedEntry = {
                        id,
                        date: antiquesEntry.date,
                        title: antiquesEntry.title,
                        content: antiquesEntry.content,
                        conversation: antiquesEntry.conversation,
                        tone: antiquesEntry.tone,
                        series: antiquesEntry.series,
                        seriesTitle: antiquesEntry.seriesTitle,
                ...
                const journalId = id.replace('tangent-', '');
                const journalEntry = (journalEntries as any[]).find(e => e.id === journalId);
                if (journalEntry?.tangentData) {
                    loadedEntry = {
                        id,
                        date: journalEntry.date,
                        title: journalEntry.title,
                        content: journalEntry.summary,
                        conversation: journalEntry.tangentData,
                        tone: journalEntry.tangentData.tone,
                        createdAt: journalEntry.tangentData.generatedAt,
                        updatedAt: journalEntry.tangentData.generatedAt,
    ...
    const toggleSavePhrase = (text: string, e: React.MouseEvent) => {
        ...
        if (savedPhrases.has(text)) {
            ...
            // Find matching Japanese line by index
            const engLines = entry?.conversation?.english || [];
            const jpLines = entry?.conversation?.japanese || [];
            const lineIdx = engLines.findIndex(l => l.text === text);
            const jpText = lineIdx >= 0 && jpLines[lineIdx] ? jpLines[lineIdx].text : undefined;
                ...
                <div style={{ padding: '8px 24px 0', maxWidth: '600px', margin: '0 auto', display: 'flex', justifyContent: 'center' }}>
                    <button
                        ...
                        style={{
                            ...
                            borderRadius: '20px',
                            border: 'none',
                            backgroundColor: completed ? t.accent : t.borderLight,
